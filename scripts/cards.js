import { Player, UnoGame } from "./gamePlay.js";
...
export class Card {
  ...
  isPlayable(topCard) {
    ...
    // Gestion des cartes +2, +4, +6, +10
    // Si la carte du dessus est une carte +X et que la carte actuelle est aussi une carte +Y
    if ((topCard.value === "+2" || topCard.value === "+4" ||
      topCard.value === "Black+4Reverse" || topCard.value === "Black+6" ||
      topCard.value === "Black+10") &&
      (this.value === "+2" || this.value === "+4" ||
        this.value === "Black+4Reverse" || this.value === "Black+6" ||
        this.value === "Black+10")) {
      // extrait les valeurs numériques des cartes "+" pour pouvoir les comparer
      // parseInt() convertit une chaîne de caractères en un entier
      // match() recherche une correspondance entre une chaîne de caractères et une expression régulière regex
      // [0] retourne la première correspondance trouvée dans la chaîne de caractères
      // || 0 si aucune correspondance est trouvée, la valeur est définie sur 0
      const topCardValue = parseInt(topCard.value.match(/\d+/)[0] || 0);
      const thisCardValue = parseInt(this.value.match(/\d+/)[0] || 0);

      // Vérifier si la carte actuelle a une valeur >= à la carte du dessus
      return thisCardValue >= topCardValue;
    }
...
export class Deck {
  ...
  initializeDeck() {
    ...
    const cardConfig = {  // objet cardConfig defini les types de carte et leur quantité,
    //  divisé en 3 sections: cartes numérotées, cartes action et cartes noires
      numbers: {
    ...
    colors.forEach((color) => {
      Object.entries(cardConfig.actions).forEach(([action, count]) => { 
        // cardConfig.actions est un objet qui définit les types de cartes d'action 
        // et leur quantité par couleur
        for (let i = 0; i < count; i++) { // Object.entries() transforme cet objet 
        // en un tableau de paires [clé, valeur]
